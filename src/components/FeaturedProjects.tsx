...
const ProjectCard = ({
  ...
  return (
    ...
    >
      {/* Image Container */}
      <div className="relative aspect-[4/3] rounded-2xl overflow-hidden border border-white/10 bg-warm-sand/5">
        <img
          src={project.image}
          alt={project.title}
        ...
        {/* Gradient Overlay */}
        <div className="absolute inset-0 bg-gradient-to-t from-near-black via-near-black/50 to-transparent opacity-60 group-hover:opacity-80 transition-opacity" />
        ...
        <AnimatePresence>
          {isHovered && (
            <motion.div
              ...
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
              className="absolute inset-0 bg-terracotta/20 backdrop-blur-sm flex items-center justify-center"
            >
              <div className="px-4 py-2 bg-terracotta rounded-full text-white text-sm font-medium flex items-center gap-2">
        ...
        {/* Category Badge */}
        <div className="absolute top-3 left-3 px-3 py-1 bg-near-black/70 backdrop-blur-md rounded-full text-[9px] font-sans font-bold uppercase tracking-widest text-ivory border border-white/10">
          {project.category}
        </div>

        {/* Locked Badge */}
        {project.locked && (
          <div className="absolute top-3 right-3 px-3 py-1 bg-red-500/70 backdrop-blur-md rounded-full text-[9px] font-sans font-bold uppercase tracking-widest text-white border border-white/10 flex items-center gap-1">
        ...
        <div className="absolute bottom-0 left-0 right-0 p-4">
          <h3 className="text-lg font-serif font-medium text-ivory mb-1 group-hover:text-terracotta transition-colors">
            {project.title}
          </h3>
          <p className="text-xs text-stone-gray line-clamp-2 group-hover:text-ivory/70 transition-colors">
            {project.description}
          </p>
        </div>
      </div>

      {/* Capabilities Bar (visible on hover) */}
      <AnimatePresence>
        {isHovered && project.capabilities && (
          <motion.div
...
export function FeaturedProjects() {
  ...
  return (
    <section className="py-16 sm:py-24 md:py-32 bg-near-black text-ivory overflow-hidden">
        ...
        <ScrollAnimation>
          <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 sm:gap-8 mb-10 sm:mb-16">
            <div className="max-w-2xl">
              ...
              <h2 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-serif font-medium leading-tight text-white">
